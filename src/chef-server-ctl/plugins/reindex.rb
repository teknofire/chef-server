  ...
  begin
    if Dir.exist?(data_dir)
      ...
      if (2.2* data_dir_size) < free_disk_space #The minimum space should be double the existing data size
        ...
        $stderr.puts "The reindex process requires at least #{2*data_dir_size} KB."
        exit 1
      end
    else
      puts "Elasticserach data path does not exist, so skipping the disk space verification: #{data_dir}"
    end
  rescue => exception
    puts "Skipping the disk space verification due to #{exception}"
  end
end
